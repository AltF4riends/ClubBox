      ...
      >
        ...
        <Stack sx={{ width: "100%" }} spacing={2}>
          ...
          <Dialog open={isPopupOpen} onClose={() => setPopupOpen(false)}>
            ...
            <DialogContent>
              <ul>
                {clubInfo.Members.map((memberID, index) => (
                  <li key={index}>
                    {/* Fetch and display member details using memberID */}
                    {/* You may need to implement a function to fetch student details by ID */}
                    {`${memberID}: Fetch and display student details here`}
                  </li>
                ))}
              </ul>
            </DialogContent>
